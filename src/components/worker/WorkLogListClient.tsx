...
export function WorkLogListClient({
  ...
  const handleMonthChange = (newMonth: { year: number; month: number }) => {
    setMonth(newMonth);
    router.push(`/worker/work-logs?year=${newMonth.year}&month=${newMonth.month}`);
  };

  const filteredLogs =
    statusFilter === "all" ? initialLogs : initialLogs.filter((log) => log.status === statusFilter);

  const approved = initialLogs.filter((l) => l.status === "approved");
  const totalHours = Math.round(approved.reduce((sum, l) => sum + l.durationHours, 0) * 100) / 100;
  ...
  return (
    <>
      ...
      {/* 상태별 탭 필터 */}
      <Tabs value={statusFilter} onValueChange={(v) => setStatusFilter(v as WorkLogStatus | "all")}>
        <TabsList>
          <TabsTrigger value="all">전체 ({initialLogs.length})</TabsTrigger>
          <TabsTrigger value="pending">대기중 ({pendingCount})</TabsTrigger>
          <TabsTrigger value="approved">승인 ({approvedCount})</TabsTrigger>
