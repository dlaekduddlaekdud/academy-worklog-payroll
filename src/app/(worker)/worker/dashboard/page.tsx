...
export default async function WorkerDashboardPage() {
  ...
  const { data: thisMonthLogs } = await supabase
    .from("work_logs")
    .select(
      "id, work_date, start_time, end_time, duration_hours, role_type, status, calculated_pay, applied_hourly_rate, submitted_at"
    )
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">예상 급여</CardTitle>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">승인된 기록</CardTitle>
      ...
      <Card>
        ...
        <CardContent>
          {recentLogs.length === 0 ? (
            <p className="py-8 text-center text-sm text-muted-foreground">근무 기록이 없습니다</p>
          ) : (
