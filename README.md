...
금액을 바꾸는 연산과 날짜 경계 계산에 집중했다. 전체 경로를 골고루 덮는 대신, **틀렸을 때 되돌릴 수 없는 지점**을 먼저 막았다.
...
**단위** (`src/lib/utils/`) — 급여 계산과 월 경계 날짜의 경계값. 2월과 윤년, 30일/31일 달이 섞이는 지점을 다룬다.

**통합** (`tests/finalize-payroll.test.ts`) — `finalize_payroll` RPC를 관리자로 실제 로그인한 클라이언트로 호출한다. service_role 키는 `auth.uid()`가 NULL이라 권한 검사 자체를 건너뛰게 되기 때문이다.

- 대기 중인 기록이 남아 있으면 확정이 거부되고, 요약 행도 남지 않는다 (단일 트랜잭션 롤백)
- 반려된 기록은 집계에서 빠진다
- 확정된 달의 근무 기록은 service_role로도 변경되지 않는다
- 재확정해도 금액이 달라지지 않는다
- 관리자가 아니면 호출할 수 없다

**E2E** (`e2e/`) — 제출 → 승인, 반려 → 수정 → 재제출, 정산 확정 세 플로우. 승인 이후 근무자 화면에서 수정 경로가 사라지는지까지 확인한다.

테스트 계정이 없으면(`TEST_ADMIN_*`, `TEST_WORKER_*`) 통합·E2E는 실패하지 않고 건너뛴다.
...
- **동시 확정 요청을 검증하지 못했다.** `finalize_payroll`은 요약 행을 `FOR UPDATE`로 잠그지만, 같은 근무자·월에 두 관리자가 동시에 호출하는 경우를 테스트로 재현하지는 않았다.
...
tests/                     finalize_payroll RPC 통합 테스트
e2e/                       승인 · 반려 · 정산 확정 플로우
scripts/                   README 화면 이미지 생성 스크립트
```

설계 과정과 단계별 작업 내역은 [ROADMAP.md](./ROADMAP.md)에 있고,
판단이 필요했던 지점과 되돌린 결정은 [docs/adr](./docs/adr)에 정리했다.
